...
def get_args() -> Namespace:
    ...
    parser.add_argument(
        "--db",
        default="./papers.db",
        help="Path to a SQLite database that the paper info will be written into",
    )
...
def save_last_page_number(con: sqlite3.Connection, last_page: int) -> None:
    cursor = con.cursor()
    cursor.execute(
        f'INSERT INTO stats ("stat", "value") VALUES ("last_page", {last_page})'
    )
    con.commit()
...
def main():
    ...
    try:
        start = None
        entries = []
        for entry, start in get_papers_in_category(
            category_id=args.category,
            start=int(last_page_number),
